...
const PortfolioSkills = ({ language }) => {
  const translations = {
    en: {
      title: "<Skills />",
      levels: {
  ...
  const skills = [
    ...
    { name: 'CSS', level: 'Intermediate', color: 'blue' },
    { name: 'CSS4', level: 'low', color: 'indigo' },
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-6">
        {skills.map((skill, index) => (
          ...
          >
            ...
            <p className="relative text-lg text-gray-200 z-10">
            ...
            <div className="absolute bottom-0 left-0 w-full h-2 bg-gray-700">
              <div
                ...
                style={{ width: skill.level === 'low' ? '25%' : skill.level === 'Advanced' ? '75%' : '50%' }}
              ></div>
            </div>
          </div>
        ))}
