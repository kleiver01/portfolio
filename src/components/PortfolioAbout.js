...
<p className="text-lg md:text-xl leading-relaxed mb-6 text-gray-200 group-hover:text-white transition-colors duration-300 typing-effect">
  {Array.from("¡Saludos, amigo! Soy un programador de experiencias digitales, un mago del código.").map((char, i) => (
    <span key={i} style={{ animationDelay: `${i * 0.05}s` }}>{char}</span>
      ...
      <p className="text-lg md:text-xl leading-relaxed text-gray-300 animate-fade-in delay-500 group-hover:text-white transition-colors duration-300">
        Mi nombre es Kleiver Uzcategui y soy programador especializado en tecnologías web. Trabajo principalmente con HTML, CSS, JavaScript y React, creando interfaces modernas, funcionales y responsivas. Me apasiona transformar ideas en experiencias digitales intuitivas, cuidando tanto la estética como el rendimiento. Siempre estoy en busca de nuevos desafíos que me permitan seguir aprendiendo y aportar soluciones creativas.
      </p>
